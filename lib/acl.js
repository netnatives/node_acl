...
Acl.prototype.addUserRoles = function(userId, roles, cb){
  this.logger?this.logger.debug('Adding roles to user: ' + userId):null;
...
Acl.prototype.removeUserRoles = function(userId, roles, cb){
  this.logger?this.logger.debug('Removing roles from user: ' + userId):null;
...
Acl.prototype.userRoles = function(userId, cb){
  this.logger?this.logger.debug('Returning user roles for user: ' + userId):null;
...
Acl.prototype.addRoleParents = function(role, parents, cb){
  this.logger?this.logger.debug('Adding parent to role: ' + role):null;
  contract(arguments)
    .params('string|number','string|array','function')
    .params('string|number','string|array')
    .end();
...
Acl.prototype.removeRole = function(role, cb){
  this.logger?this.logger.debug('Removing role: ' + role):null;
  contract(arguments)
...
Acl.prototype.removeResource = function(resource, cb){
  this.logger?this.logger.debug('Removing resource: ' + role):null;
  contract(arguments)
    .params('string', 'function')
    .params('string')
    .end();
...
Acl.prototype.middleware = function(numPathComponents, userId, actions){
  ...
  // acl.logger?acl.logger.debug('Returning middleware'):null;
  ...
  return function(req, res, next){
    ...
    // acl.logger?acl.logger.debug('Middleware callback has been called'):null;
    ...
    if (!userId) {
      if((req.session) && (req.session.user) && (req.session.user._id)){
        ...
        next(new restify.InvalidCredentialsError('User not authenticated'));
        return;
      }
    }

    url = (req.route && req.route.path) ? req.route.path : req.url.split('?')[0];
    ...
    acl.isAllowed(_userId, resource, actions, function(err, allowed){
      if (err){
        next(new restify.InternalError('Error checking permissions to access resource'));
        ...
        next(new restify.NotAuthorizedError('Insufficient permissions to access resource'));
      }else{
